...
export class Packet {
   ...
   */
  headerToObject(buf) {
    const obj: Writeable<Partial<HeaderPacket>> = {};
    let offset = 0;
    ...
    obj.ackRequired =
      (frameAddressDescription & constants.ACK_REQUIRED_BIT) !== 0;
    obj.resRequired =
      (frameAddressDescription & constants.RESPONSE_REQUIRED_BIT) !== 0;
    offset += 1;
   ...
   */
  toObject(buf) {
    let obj: Writeable<Partial<HeaderPacket>> = {};
    ...
    if (obj.type !== undefined) {
      ...
      if (packets[typeName] !== undefined) {
        if (typeof packets[typeName].toObject === "function") {
          const specificObj = packets[typeName].toObject(
            buf.slice(constants.PACKET_HEADER_SIZE)
          );
   ...
   */
  create(type, params, source, target) {
    const obj: Writeable<Partial<HeaderPacket>> = {};
    if (type !== undefined) {
