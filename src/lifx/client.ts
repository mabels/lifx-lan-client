...
export class Client extends EventEmitter {
  ...
  init(options, callback) {
    ...
    if (typeof opts.port !== "number") {
      ...
      throw new RangeError(
        "LIFX Client port option must be between 0 and 65535"
      );
    }
    ...
    if (typeof opts.lightOfflineTolerance !== "number") {
      throw new TypeError(
        "LIFX Client lightOfflineTolerance option must be a number"
      );
    }
    ...
    if (typeof opts.messageHandlerTimeout !== "number") {
      throw new TypeError(
        "LIFX Client messageHandlerTimeout option must be a number"
      );
    }
    ...
    if (typeof opts.resendPacketDelay !== "number") {
      throw new TypeError(
        "LIFX Client resendPacketDelay option must be a number"
      );
    }
    ...
    if (typeof opts.broadcast !== "string") {
      ...
      throw new TypeError(
        "LIFX Client broadcast option does only allow IPv4 address format"
      );
    }
    ...
    if (typeof opts.sendPort !== "number") {
      ...
      throw new RangeError(
        "LIFX Client sendPort option must be between 1 and 65535"
      );
    }
    ...
    if (!isArray(opts.lights)) {
      ...
      opts.lights.forEach(function(light) {
        if (!utils.isIpv4Format(light)) {
          throw new TypeError(
            "LIFX Client lights option array element '" +
              light +
              "' is not expected IPv4 format"
          );
        }
      });
    ...
    if (opts.source !== "") {
      if (typeof opts.source === "string") {
        ...
        throw new TypeError(
          "LIFX Client source option must be given as string"
        );
      }
    }
    ...
    if (typeof opts.messageRateLimit !== "number") {
      throw new TypeError(
        "LIFX Client messageRateLimit option must be a number"
      );
    } else if (opts.messageRateLimit <= 0) {
      throw new RangeError(
        "LIFX Client messageRateLimit option must greater than 0"
      );
    }
    ...
    if (typeof opts.discoveryInterval !== "number") {
      throw new TypeError(
        "LIFX Client discoveryInterval option must be a number"
      );
    } else if (opts.discoveryInterval <= 0) {
      throw new RangeError(
        "LIFX Client discoveryInterval option must greater than 0"
      );
    }
    ...
    this.socket.on(
      ...
      function(msg, rinfo) {
        ...
        if (
          utils.getHostIPs().indexOf(rinfo.address) >= 0 ||
          !Buffer.isBuffer(msg)
        ) {
          return;
        }
        ...
        if (this.debug) {
          console.log(
            "DEBUG - " + msg.toString("hex") + " from " + rinfo.address
          );
        }
        ...
        if (parsedMsg instanceof Error) {
          ...
          const messageTypeName = result(
            find(Packet.typeList, { id: parsedMsg.type }),
            "name"
          );
  ...
  sendingProcess(queueAddress) {
    ...
    return () => {
      ...
      if (messageQueue.length > 0) {
        ...
        if (
          msg.transactionType === constants.PACKET_TRANSACTION_TYPES.ONE_WAY
        ) {
          this.socket.send(
            msg.data,
            0,
          ...
          msg.transactionType ===
          constants.PACKET_TRANSACTION_TYPES.REQUEST_RESPONSE
        ) {
          if (msg.timesSent < this.resendMaxTimes) {
            ...
            this.messageHandlers.forEach(
              function(handler, hdlrIndex) {
                if (
                  handler.type === "acknowledgement" &&
                  handler.sequenceNumber === msg.sequence
                ) {
                  this.messageHandlers.splice(hdlrIndex, 1);
                  const err = new Error(
                    "No LIFX response after max resend limit of " +
                      this.resendMaxTimes
                  );
  ...
  startSendingProcess(queueAddress = this.broadcastAddress) {
    if (isNil(this.sendTimers[queueAddress])) {
      ...
      this.sendTimers[queueAddress] = setInterval(
        sendingProcess,
  ...
  predefinedDiscoveredAndOnline() {
    const predefinedDevices = filter(this.devices, device =>
      includes(this.lightAddresses, device.address)
    );

    const numDiscovered = keys(this.devices).length;
    const allDiscovered = numDiscovered >= this.lightAddresses.length;
    const allOnline = every(
      predefinedDevices,
      device => device.status === "on"
    );
    const labelsReceived = every(predefinedDevices, device =>
      isString(device.label)
    );
  ...
  addMessageHandler(type, callback, sequenceNumber) {
    if (typeof type !== "string") {
      throw new TypeError(
        "LIFX Client addMessageHandler expects type parameter to be string"
      );
    }
    if (typeof callback !== "function") {
      throw new TypeError(
        "LIFX Client addMessageHandler expects callback parameter to be a function"
      );
    }
    ...
    if (typeName === undefined) {
      throw new RangeError(
        "LIFX Client addMessageHandler unknown packet type: " + type
      );
    }
    ...
    if (sequenceNumber !== undefined) {
      if (typeof sequenceNumber !== "number") {
        throw new TypeError(
          "LIFX Client addMessageHandler expects sequenceNumber to be a integer"
        );
      } else {
  ...
  lights(status) {
    ...
    if (status.length > 0) {
      if (status !== "on" && status !== "off") {
        throw new TypeError(
          "Lifx Client lights expects status to be 'on', 'off' or ''"
        );
      }
  ...
  light(identifier) {
    ...
    if (typeof identifier !== "string") {
      throw new TypeError(
        "LIFX Client light expects identifier for LIFX light to be a string"
      );
    }
